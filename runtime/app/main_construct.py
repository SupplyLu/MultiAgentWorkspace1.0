"""Construct Runtime 守护进程入口

负责：
  - 启动 ConstructRuntime + Signal Server
  - 轮询 Queue 目录，自动派发任务
  - 超时检测（基于任务 TIMEOUT 字段）
  - 日志输出到文件和 stdout
  - 优雅关闭（SIGINT/SIGTERM）
  - 动态扫描 constructor_* 槽位目录
"""
...
def find_free_port(start: int = 19020, end: int = 19120) -> int:
    """查找可用端口（与 Thinking Runtime 18910-19010 隔开）"""
...
def setup_logging(log_file: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("construct_runtime")
...
def ensure_directory_structure(root_dir: Path) -> None:
    """确保必要的目录结构存在，动态识别所有 constructor_* 槽位"""
    pools_dir = root_dir / "pools" / "construct"
    (pools_dir / "Queue").mkdir(parents=True, exist_ok=True)
    (pools_dir / "Outbox").mkdir(parents=True, exist_ok=True)

    # 动态扫描所有 constructor_* 目录并创建 workspace
    if pools_dir.exists():
        for sub_dir in pools_dir.iterdir():
            if sub_dir.is_dir() and fnmatch.fnmatch(sub_dir.name, "constructor_*"):
                (sub_dir / "workspace").mkdir(parents=True, exist_ok=True)


def main() -> None:
    """主函数"""
    parser = argparse.ArgumentParser(description="Construct Runtime 守护进程")
    parser.add_argument(
        "root_dir",
        nargs="?",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="信号服务端口（默认动态分配 19020-19120）",
    )
    parser.add_argument(
        "--poll-interval",
        type=float,
    ...
    log_file = root_dir / "logs" / "construct_runtime.log"

    # 初始化日志
    logger = setup_logging(log_file)
    logger.info("=" * 50)
    logger.info("Construct Runtime 守护进程启动")
    ...
    # 初始化 ConstructRuntime
    runtime = ConstructRuntime(root_dir=root_dir, signal_port=signal_port)
    runtime.start()
