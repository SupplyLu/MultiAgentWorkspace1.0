#!/usr/bin/env python
"""Gate full batch closed-loop script with real Construct-style output."""
...
def main() -> int:
    ...
    batch_id = "batch_t_pmsm_foc_sim_002"
    batch_dir = queue_dir / batch_id
    ...
    if not batch_dir.exists():
        ...
        (batch_dir / "task_t_pmsm_w001.txt").write_text(
            "FROM: construct\nTO: gate\n---\nWork task 1",
            encoding="utf-8",
        )
        (batch_dir / "task_t_pmsm_w002.txt").write_text(
            "FROM: construct\nTO: gate\n---\nWork task 2",
            encoding="utf-8",
        )
        (batch_dir / "project_blueprint.md").write_text("# Test Blueprint", encoding="utf-8")
    ...
    if not result.get("dispatched"):
        payload["error"] = result.get("error", "Unknown dispatch error")
    ...
    if slot is None:
        payload["error"] = f"Slot not found: {slot_id}"
        print(json.dumps(payload, ensure_ascii=False, indent=2))
        return 1

    task_id = result["task_id"]
    fields_input = gate_pool / "fields" / task_id / "input"
    slot_reference_task = gate_pool / slot_id / f"task_{task_id}.txt"
    ...
    for task_file in work_task_files:
        (slot.workspace_dir / task_file.name).write_text(task_file.read_text(encoding="utf-8"), encoding="utf-8")
    ...
    outbox_task_1 = outbox_dir / "task_t_pmsm_w001.txt"
    outbox_task_2 = outbox_dir / "task_t_pmsm_w002.txt"

    payload["step5_outbox_task1_exists"] = outbox_task_1.exists()
    payload["step5_outbox_task2_exists"] = outbox_task_2.exists()
    ...
    payload["step5_field_cleaned_up"] = not (gate_pool / "fields" / task_id).exists()
